...
FRUIT_COLOR = (FRUIT_R, FRUIT_G, FRUIT_B)
BUTTERFLY_COLOR = (BUTTERFLY_R, BUTTERFLY_G, BUTTERFLY_B)

# Background screens to cycle through:
SCREEN_COLORS = [
    (10, 10, 30),
    (30, 10, 30),
    (10, 30, 30)
...
# When spawning a tree, use smaller lengths and depths.
# For chaotic, shorter trees, length in [80, 120], width in [10, 20], max_depth 2 or 3.
...
class Tree:
    ...
    def generate_children(self, parent, depth):
        if depth <= 0:
            ...
            return
        # Increase chaos by allowing a wide range of random angle offsets.
        if depth > 1:
    ...
    def update(self, current_time, speed_factor=1.0):
        """
        Add one branch from the pending list if enough time has passed.
        The effective delay is adjusted by the speed factor.
        """
        ...
        if self.pending_branches and (current_time - self.last_branch_growth_time >= effective_delay):
            ...
            # Accelerate growth by reducing the delay (down to a minimum)
...
class Fruit:
    ...
    def __init__(self, position):
        ...
        # Precompute base HLS from FRUIT_COLOR.
        r, g, b = FRUIT_COLOR
    ...
    def draw(self, surface):
        # The overall scale of the fruit increases with growth.
        current_size = self.size * self.growth
        # Adjust saturation as it grows.
        ...
        # Draw each trapezoid from bottom (part 0) upward.
        for i in range(num_parts):
            # The bottom width shrinks a little with each part.
...
class Butterfly:
    ...
    def update(self, screen_width, screen_height):
        self.wing_flap_timer += 1.0  # Fast flapping
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if state == 0:
                    state = 1
                    # Start with one tree growing from the bottom-center.
                    pos, angle = get_random_tree_spawn()  # Could be any edge
                    length, width, max_depth = get_random_tree_params()
                    center_tree = Tree(pos, angle, length, width, max_depth)
    ...
    if state == 1:
        ...
        for tree in trees:
            ...
            tree.draw(screen)
        # Spawn new trees from random edges at a moderate rate.
